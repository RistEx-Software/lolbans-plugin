			...
			public Optional<ResultSet> call()
			{
				...
				try 
				{
					...
					for (IPAddressString cb : LolBans.BannedAddresses)
					{
						...
						{
							PreparedStatement pst = Database.connection.prepareStatement("SELECT * FROM lolbans_ipbans WHERE IPAddress = ? AND Appealed = false"); 
		...
		FutureTask<UUID> t = new FutureTask<>(new Callable<UUID>()
		{
			...
			public UUID call()
			{
				...
				try
				{
					...
					ResultSet results = Database.connection.prepareStatement("SELECT UUID,IPAddress FROM lolbans_punishments WHERE Type = 0 AND Appealed = False").executeQuery();
